...
Hub.app = {
  ...
  async init() {
    ...
    // ── If returning from OAuth, wait for PKCE code exchange ──
    // This prevents the classic "login → back to login" loop where the app
    // decides "no session" before Supabase finishes exchanging ?code=...
    if (Hub.oauth?.returnedFromOAuth) {
      console.log('[Auth] OAuth redirect detected. Waiting for exchange…');
      try {
        await Promise.race([
          Hub.oauth.promise,
          new Promise((res) => setTimeout(res, 10000))
        ]);
        console.log('[Auth] OAuth exchange done:', {
          ok: Hub.oauth.exchangeOk,
          err: Hub.oauth.exchangeError
        });
      } catch (e) {
        console.warn('[Auth] OAuth wait exception:', e.message);
      }
    }
// ── Auth listener ──
    ...
    Hub.auth.onAuthChange(async (event, session) => {
      ...
      // CRITICAL: If already logged in and showing dashboard, ignore ALL events except actual sign out
      ...
      if (event === 'INITIAL_SESSION' && !session) {
        // If we JUST returned from OAuth, don't immediately punt to login.
        // Give getSession() fallback a chance (or show OAuth error if exchange failed).
        if (Hub.oauth?.returnedFromOAuth) {
          console.warn('[Auth] INITIAL_SESSION: no session after OAuth redirect');
          if (Hub.oauth.exchangeError) {
            Hub.ui?.banner?.('OAuth error: ' + Hub.oauth.exchangeError, 'error');
            const errEl = document.getElementById('loginError');
            if (errEl) {
              errEl.textContent = 'OAuth error: ' + Hub.oauth.exchangeError;
              errEl.style.display = 'block';
            }
          }
          // Do not mark authHandled; allow fallbacks to run getSession()
          return;
        }
    ...
    // Fallback 1: 5s — try getSession manually
    setTimeout(async () => {
      if (this._loggedIn || this._authHandled || this._loginInProgress) {
        console.log('[Auth] 5s fallback skipped (already handled)');
        return;
      }
      console.log('[Auth] 5s fallback — getSession()');
      try {
        const session = await Hub.auth.getSession();
        console.log('[Auth] 5s fallback session:', session?.user?.email || 'none');
        ...
        console.error('[Auth] 5s fallback error:', e);
        if (!this._loggedIn && !this._loginInProgress) Hub.router.showScreen('login');
      }
    }, 5000);

    // Fallback 2: 15s — absolute safety net (DISABLED if logged in)
    setTimeout(() => {
      if (this._loggedIn) {
        console.log('[Auth] 15s fallback skipped - already logged in');
        return;
      }
      if (!this._loggedIn && !this._loginInProgress) {
        const el = document.getElementById('loadingScreen');
        if (el && el.style.display !== 'none') {
          console.warn('[Auth] 15s HARD fallback → login');
          Hub.router.showScreen('login');
        }
      }
    }, 15000);
