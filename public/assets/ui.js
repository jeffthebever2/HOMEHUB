...
Hub.ui = {
  ...
  /** Normalize NWS/custom severity string → CSS class */
  _bannerClass(severity) {
    const s = (severity || '').toLowerCase();
    if (s.includes('extreme') || s.includes('severe') || s === 'warning')  return 'warning';
    if (s.includes('moderate'))                                              return 'moderate';
    if (s.includes('minor') || s.includes('advisory') || s.includes('statement')) return 'advisory';
    if (s.includes('watch'))                                                 return 'watch';
    if (s.includes('info'))                                                  return 'info';
    return 'watch'; // default amber
  },
  ...
  showBanner(text, severity) {
    ...
    if (!banner) return;
    const cls = this._bannerClass(severity);
    banner.className = 'alert-banner ' + cls;
    // Use named span so JS-generated banners work too
    const textEl = Hub.utils.$('alertBannerText');
    if (textEl) {
      textEl.textContent = text;
    } else {
      // Fallback: banner might not have the inner span (old HTML)
