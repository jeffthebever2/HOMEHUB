...
Hub.app = {
  ...
  async init() {
    ...
    // Register auth listener FIRST — this catches the OAuth redirect callback
    ...
    // Give the OAuth callback a moment to process, then fallback to login
    setTimeout(() => {
      if (!authResolved && !Hub.state.user) {
        console.log('[Auth] No session after timeout, showing login');
        Hub.router.showScreen('login');
      }
    }, 2000);
