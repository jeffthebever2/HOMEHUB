...
- The shell already degraded in some cases, but auth boot and settings diagnostics were not explicit enough for local setup or Vercel deployment.
...
- Settings now exposes lightweight Supabase diagnostics, `vercel.json` prevents stale `config.js` caching, and the README now has a dedicated `Supabase setup` section.
